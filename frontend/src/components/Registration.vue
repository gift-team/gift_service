<template>
  <div class="container-fluid p-0 text-center">
    ...
    <div class="registration pt-5 pb-5">
      <div class="regHeader">
        <p>РЕГИСТРАЦИЯ</p>
      </div>
      <div class="regForm d-flex">
        ...
        <form v-on:submit.prevent="submitForm" class="text-center col-6 p-0" action="#">
          ...
          <input
            ...
            class="password2 col-12"
            type="password">
          <span class="text-danger d-flex" v-if="!matchPass() && password">Пароли не совпадают</span>
          ...
          <div class="lendBts p-0 col-12 flex-column flex-md-row d-flex justify-content-md-between align-items-center">
            <button v-if="matchPass()" class="regBtnTitle">РЕГИСТРАЦИЯ</button>
            <div style="background: gray" v-else class="d-flex align-items-center justify-content-center regBtnTitle">РЕГИСТРАЦИЯ</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			registerUser() {
				Users.create({'email': this.email, 'password': this.password}).then(() => {
